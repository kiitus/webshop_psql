...
function App() {
  ...
  const [message, setMessage] = useState(null)  //Show messages when adding product to order
  ...
  const addToOrder = (id, text, price, amount) => {
    ...
    if (isAready) {
      ...
      setMessage(`Product: ${text} amount was updated to ${amount}`)

       
    }
    //If new product to order
    else {
      setOrder(order.concat({ id, text, price, amount }))

      setMessage(`Product: ${text} Amount: ${amount} was added to order`)
    }
